...
# Initialize manuf parser
parser = manuf.MacParser()

def guess_device_type(mac, hostname=None):
    mac_prefix = mac.upper()[0:8].replace(":", "-")

    # Try manuf vendor first
    vendor = parser.get_manuf(mac)
    if vendor:
        vendor = vendor.strip()

        # Router vendors
        router_vendors = ["Cisco", "Netgear", "Linksys", "TP-Link", "Asus", "D-Link", "Belkin", "Ubiquiti", "Google"]
        for rv in router_vendors:
            if rv.lower() in vendor.lower():
                return f"{vendor} Router"

        # Mobile vendors
        mobile_vendors = ["Apple", "Samsung", "Xiaomi", "Huawei", "OnePlus", "Sony", "LG", "Motorola", "HTC", "Google"]
        for mv in mobile_vendors:
            if mv.lower() in vendor.lower():
                return f"{vendor} Mobile Device"

        # Laptop vendors
        laptop_vendors = ["Dell", "HP", "Lenovo", "Asus", "Acer", "Microsoft"]
        for lv in laptop_vendors:
            if lv.lower() in vendor.lower():
                return f"{vendor} Laptop"

        # Virtual machines
        if "VMware" in vendor or "Virtual" in vendor:
            return "Virtual Machine"

        if "Raspberry" in vendor:
            return "Raspberry Pi"

        # If none matched, return vendor
        return vendor

    # Fallback known prefixes for common devices
    known_types = {
    ...
    # Hostname hints for mobile/laptop/router
